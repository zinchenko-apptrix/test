...
def approve_contract(w3, private_key, network, adr_dict, my_address):
    try:
        ...
        if allowance == approve_amount:
            return True
        contract_txn = contract.functions.approve(adr_dict['spender_adr'], approve_amount).build_transaction({
            'from': my_address,
            'value': 0,
...
def send_to_aptos(w3, private_key, my_address, amount):
    try:
        ...
        fee = contr.functions.quoteForSend(call_params, adapter_params).call()
        ...
        logger.info(f"{my_address} | УСПЕШНО отправка {amount / 10**8} USDC  tx {txn_hash.hex()}")
    except Exception as error:
        logger.error(f"{my_address} | НЕУДАЧНО отправка USDC | {error}")


def bridge_to_aptos(w3, private_key, network):
